#include <iostream>
using namespace std;
// read A Period, then Print its days Length.
...
short GetDifferenceInDays(stDate Date1, stDate Date2,
  ...
  return WithLastDay ? ++N : N;
}

short CalculatePeroidLength(stPeriod Period, bool WithLastDay = false) {
...
int main() {
  ...
  cout << "\nPeroid Length is: " << CalculatePeroidLength(Period);
  cout << "\nWith Last Day: " << CalculatePeroidLength(Period, true);
